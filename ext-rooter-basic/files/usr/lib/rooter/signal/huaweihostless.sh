...
handle_timeout "$!"
local in_mod="`cat $TMPFILE | grep \"<DeviceName>\" | cut -d \">\" -f 2 | cut -d \"<\" -f 1`"
if [ "$in_mod" != "" ]; then
	MODEM=$MANUF" "$in_mod
fi
IMEI="Unknown"
...
handle_timeout "$!"
COPS=""
local in_provider="`cat $TMPFILE | grep \"<FullName>\" | cut -d \">\" -f 2 | cut -d \"<\" -f 1`"
...
COPS_MCC="-"
COPS_MNC="-"
local in_mcc="`cat $TMPFILE | grep \"<Numeric>\" | cut -d \">\" -f 2 | cut -d \"<\" -f 1`"
if [ "$in_mcc" != "" ]; then
	COPS_MCC=${in_mcc:0:3}
	COPS_MNC=${in_mcc:3:3}
fi
